...
import { Loader2, Plus, Calendar, ExternalLink, MoreVertical, Trash2 } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const fetchKits = async () => {
    try {
      ...
      const sortedKits = (data || []).sort((a: Kit, b: Kit) => 
            ...
            >
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-2 text-sm text-slate-400 mb-4">
                  <span>{kit.company_brief.company_name}</span>
                  <span>•</span>
                  <span className="flex items-center gap-1 hover:text-indigo-400 transition-colors">
                    <ExternalLink className="w-3 h-3" />
                    <span className="truncate max-w-[100px]">{new URL(kit.company_url).hostname}</span>
                  </span>
                </div>
              </div>
